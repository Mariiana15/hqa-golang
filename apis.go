...
func Dispatcher(ws *websocket.Conn, m Message) {
	...
	case "hqatasks":
		log.Println("Get task hqa")
		handelGetIndustryTask(ws, m)
	case "hqacontext":
		log.Println("Get task hqa")
		handelGetStoryContextTask(ws, m)
	case "hqaoperation":
		log.Println("Get task hqa")
		handelGetOperationContextTask(ws, m)
	case "select":
...
func handelGetIndustryTask(ws *websocket.Conn, m Message) {
	fmt.Println(m.Message.Body)
	var rop requestOpenAI
	rop.Id = m.Message.ObjectId
	rop.Text = m.Message.Body
	var rq http.Request
	_, errIndy := GetIndustryOIA(&rq, &rop)
	if errIndy != nil {
		handlerExceptionWS(ws, errIndy.Error(), StateKO)
		ws.Close()
		return
	}
	ws.WriteJSON(true)
...
func handelGetStoryContextTask(ws *websocket.Conn, m Message) {
	fmt.Println(m.Message.Body)
	var rop requestOpenAI
	rop.Id = m.Message.ObjectId
	rop.Text = m.Message.Body
	var rq http.Request
	_, errContext := GetUserStoryContext(&rq, &rop)
	if errContext != nil {
		handlerExceptionWS(ws, errContext.Error(), StateKO)
		ws.Close()
		return
	}
	ws.WriteJSON(true)
...
func handelGetOperationContextTask(ws *websocket.Conn, m Message) {
	fmt.Println(m.Message.Body)
	var rop requestOpenAI
	rop.Id = m.Message.ObjectId
	rop.Text = m.Message.Body
	var rq http.Request
	_, errOperation := GetOperationContext(&rq, &rop)
	if errOperation != nil {
		handlerExceptionWS(ws, errOperation.Error(), StateKO)
		ws.Close()
		return
	}
	ws.WriteJSON(true)
